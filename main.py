...
def summarize_news(sections, openai_api_key):
    """Summarize news articles by sections using OpenAI in two steps"""
    ...
    for section_name, articles in sections.items():
        ...
        try:
            ...
            selected_headlines = response.choices[0].message.content.strip().split('\n')
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[{"role": "user", "content": prompt_summarize}],
            max_tokens=1500,
            temperature=0.3
        )
