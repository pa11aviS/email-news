...
def summarize_news(sections, api_key):
    ...
    for section_name, articles in sections.items():
        ...
        From the following headlines in {section_name.upper()}, select the top 5-7 most relevant stories based on significance, recency, and interests (technology, finance, politics, art/culture, sports for Sports News). Do NOT include sports except in Sports News.
    ...
    prompt_summarize = f"""You are a journalist curating an email newsletter for a high-paying professional audience. Summarize the selected news into these sections: AI NEWS, MAJOR INTERNATIONAL NEWS, AUSTRALIAN NEWS, SPORTS NEWS, TRENDING ON SOCIAL MEDIA.

    For each section, provide 5-7 bullet points with title, brief summary (1-2 sentences), and source link. Ensure at least 5 items per section. Format with HTML: <h2> headers, <ul><li> bullets, bold titles.
...
def main():
    ...
    sections['Major International News'] = fetch_newsapi_news(config['newsapi_key'], query='world news OR international OR politics', days_back=1, sources='bbc-news,nytimes,the-guardian')
